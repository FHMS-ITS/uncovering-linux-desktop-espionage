# This module attempts to identify all clients that capture keyboard events via X.org core events.
...
class XorgCapturingClients(interfaces.plugins.PluginInterface):
    """Lists the clients of an X.org server that capture keyboard or mouse events by registering for X core event notifications."""
            ...
            while ptr + struct_size < region_end:
                ...
                if screen is not None:
                    ...
                    for window_sibling in window_siblings:
                        try:
                            for client in window_sibling.optional.dereference().otherClients.dereference():
                                ...
                                client.client_index = ((client.resource & RESOURCE_CLIENT_MASK)) >> CLIENTOFFSET
                                # KeyPressMask (1<<0) -> 0x01                    
                                # KeyReleaseMask (1<<1) -> 0x02
                                if (client.mask & 0x03):
                                    vollog.info(f"Found client: 0x{client.client_index} capturing events with event mask 0x{client.mask:x}")
                                    yield client
    ...
    def _generator(self, tasks):
        ...
        for task in tasks:
            ...
            # call extract method
            capture_procs = self.extract(proc_layer_name, task)
            result.extend(capture_procs)

        return result


    def run(self):
        vollog.info("Volatility 3 - Identify X.org server capturing clients (core events).")
        ...
        # get the clients that registered for events
        ...
        for c in clients:
            ...
            enriched_clients.append((0,(matching_info[0].client_pid, matching_info[0].cmdname, f"{c.client_index}", f"0x{c.mask:x}", event_type)))
